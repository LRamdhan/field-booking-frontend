...
const AboutVisiMisiAnimation = ({children, marginView, ...others}) => {
  const target = useRef()
  ...
  useEffect(() => {
    ...
    const initialY = '50%'
    const intialYTitle = '80%'
    const initialVectorScale = 0.5

    titleElement.style.transform = `translateY(${intialYTitle})`
    ...
    inView(target.current, () => {
      const sequence = [
        ...
        [
          ...
          {
            opacity: [0, 1],
            transform: [`scale(${initialVectorScale})`, `scale(1)`]
          }, {
            at: 0.7,
            duration: 1,
            ease: "easeOut",
...
export default AboutVisiMisiAnimation